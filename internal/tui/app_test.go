...
func TestProgramStartsDarwinMainMenu(t *testing.T) {
	final := runProgram(t, NewApp(Dependencies{
		Platform: platform.Platform{OS: platform.Darwin, Variant: platform.Native},
		Assets:   testAssets{},
	}), "\x1b[B\x1b[B\x1b[B\x1b[B\r")

	if view := final.View().Content; !strings.Contains(view, "MyDots") {
		t.Errorf("final model view = %q, want main menu", view)
	}
}
...
func TestProgramShowsWindowsGuideAndExitsOnKey(t *testing.T) {
	...
	if view := final.View().Content; !strings.Contains(view, "Install WSL2 with: wsl --install") {
		t.Errorf("final model view = %q, want Windows guide", view)
	}
	if installStarts != 0 {
...
func TestProgramMainMenuGolden(t *testing.T) {
	...
	finalApp, ok := final.(App)
	if !ok {
		t.Fatalf("final model = %T, want tui.App", final)
	}
	if _, ok := finalApp.model.(screens.MainMenu); !ok {
		t.Fatalf("final app screen = %T, want screens.MainMenu", finalApp.model)
	}
	...
	if view != string(want) {
		t.Errorf("final model view = %q, want golden %q", view, want)
	}
}
...
func runProgram(t *testing.T, model tea.Model, input string) tea.Model {
	t.Helper()
