...
- [ ] 1.2 **T-074** Audit `assets/scripts/omz-install.sh` for script-owned `RUNZSH=no CHSH=no`; RED only for a gap, minimally GREEN, verify no prompts/failure/cleanup with fakes, ShellCheck, then update only T-074 with tests and docs in its atomic WHY commit.
