...
Every behavior MUST have a failing RED test before GREEN. Unit behavior MUST use direct model manipulation; teatest MUST cover only integral full-program behavior; visual regression MUST use fixed deterministic goldens. Tests MUST NOT depend on network, root, real HOME, or host state. Phase 5 MUST NOT change Phase 4, create Phase 6 content, or define Phase 7 CLI/entry-point flags. Future commits MUST be conventional, T-xxx-aligned, and explain decisions.
