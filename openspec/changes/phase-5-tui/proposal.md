...
### Out of Scope
- Phase 4 pipeline/status-contract changes; Phase 6 cheatsheet creation; Phase 7 CLI/entry-point flags.
...
Add layered `internal/tui` screen models and reusable components. Inject config, backup, platform, embedded-asset, executor, and cancellation seams; map the executor's literal `running` status in the renderer without altering Phase 4. Work strictly RED then GREEN for each T-081–T-096 pair: direct `Update`/`View` tests for units, fixed-dimension golden files for renders, and teatest only for full-program T-093 behavior. Future commits are conventional, one per T-xxx pair, with decision rationale in the body.
...
| `internal/config/**`, `internal/backup/**`, `internal/installer/**` | Consumed | Injected boundaries; no Phase 4 changes |
...
Revert the Phase 5 T-xxx commits; no installer pipeline, persisted-data migration, or Phase 6/7 behavior is changed.
