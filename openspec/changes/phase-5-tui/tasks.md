...
- [ ] Run each RED test demonstrably failing, then narrow and full `go test ./...`; goldens update only via `-update` then rerun normally.
- [ ] Commit each pair with tests and behavior in one conventional T-xxx-pair commit whose body states the decision rationale; no Phase 4/6/7 edits, network, root, real HOME, or host state.
