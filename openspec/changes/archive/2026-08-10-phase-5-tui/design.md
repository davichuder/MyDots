...
Implement T-081–T-096 as Bubble Tea v2 models in `internal/tui`. `App` routes typed outcomes; commands own I/O; updates are deterministic. Inject config, backup, platform, assets, installation, elevation, terminal, clock, and render seams. `main.go`, Phase 6 assets, and Phase 7 behavior remain unchanged. The approved Phase 5 integration seam is limited to canonical executor cancellation and module-name progress metadata.
...
| Canonical session adapter | Preserves installer semantics | `SessionRunner` delegates to `installer.Run`; `Run` checks cancellation before each next module and publishes `Module.Name()` with the typed ID. |
...
| Native Bubble Tea v2 harness | Compatible integral coverage | `NewApp(Dependencies)` covers Darwin startup and Windows guide/exit only; v1-only teatest is retained only as feasibility RED evidence. |
...
`startInstall` creates one Phase-5 session context and is its sole cancellation owner. Cancellation marks the session, invokes its cancel function, stops elevation, and schedules no further runner step. The production `SessionRunner` builds the existing `installer.BuildPlan` and delegates to canonical `installer.Run`. The approved executor seam checks `ctx.Err()` before each next module while passing the same context to `Install`; therefore cancellation between non-critical modules prevents every later module from starting or mutating the host, while the currently running subprocess receives cancellation. `ProgressEvent` carries `Module.Name()` with its typed `ModuleID`, so rows render human names without changing typed ID routing or status aggregation. This is a narrow Phase 5 integration seam, not broad Phase 4 scope. It emits typed progress, log, and one completion event.
...
| `internal/installer/executor.go` | Modify | Approved cancellation-before-next-module guard and module-name progress metadata for canonical adapter delegation. |
| `main.go`, `assets/**`, `internal/{backup,config}/**` | Unchanged | Phase boundary. |
...
| Integral | Darwin menu; Windows guide/exit | Injected native Bubble Tea v2 `Program`; no host signals, traces, or terminal I/O. |
